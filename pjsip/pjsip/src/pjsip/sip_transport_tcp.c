                                        ...
                                        pjsip_transport **p_transport)
{
    ...
    //add by james
    #if defined(PJ_SOCKET_KEEP_ALIVE_ENABLE) && PJ_SOCKET_KEEP_ALIVE_ENABLE!=0
    pj_sockopt_params *ka_sockopt_params = &listener->sockopt_params;
    if (!ka_sockopt_params->cnt) {
        static int ka_enable   = PJ_SOCKET_KEEP_ALIVE_ENABLE;
        static int ka_idle     = PJ_SOCKET_KEEP_ALIVE_IDLE;
        static int ka_interval = PJ_SOCKET_KEEP_ALIVE_INTERVAL;
        static int ka_count    = PJ_SOCKET_KEEP_ALIVE_COUNT;
        static int tcp_timeout = PJ_SOCKET_TCP_USER_TIMEOUT * 1000;
        int opt_idx = 0;
        if (opt_idx < PJ_MAX_SOCKOPT_PARAMS) {
            ka_sockopt_params->options[opt_idx].level   = pj_SOL_SOCKET();
            ka_sockopt_params->options[opt_idx].optname = SO_KEEPALIVE;
            ka_sockopt_params->options[opt_idx].optval  = &ka_enable;
            ka_sockopt_params->options[opt_idx].optlen  = sizeof(ka_enable);
        }
        opt_idx++;
        if (opt_idx < PJ_MAX_SOCKOPT_PARAMS) {
            ka_sockopt_params->options[opt_idx].level   = pj_SOL_TCP();
            ka_sockopt_params->options[opt_idx].optname = TCP_KEEPIDLE;
            ka_sockopt_params->options[opt_idx].optval  = &ka_idle;
            ka_sockopt_params->options[opt_idx].optlen  = sizeof(ka_idle);
        }
        opt_idx++;
        if (opt_idx < PJ_MAX_SOCKOPT_PARAMS) {
            ka_sockopt_params->options[opt_idx].level   = pj_SOL_TCP();
            ka_sockopt_params->options[opt_idx].optname = TCP_KEEPINTVL;
            ka_sockopt_params->options[opt_idx].optval  = &ka_interval;
            ka_sockopt_params->options[opt_idx].optlen  = sizeof(ka_interval);
        }
        opt_idx++;
        if (opt_idx < PJ_MAX_SOCKOPT_PARAMS) {
            ka_sockopt_params->options[opt_idx].level   = pj_SOL_TCP();
            ka_sockopt_params->options[opt_idx].optname = TCP_KEEPCNT;
            ka_sockopt_params->options[opt_idx].optval  = &ka_count;
            ka_sockopt_params->options[opt_idx].optlen  = sizeof(ka_count);
        }
        opt_idx++;
        if (opt_idx < PJ_MAX_SOCKOPT_PARAMS) {
            ka_sockopt_params->options[opt_idx].level   = pj_SOL_TCP();
            ka_sockopt_params->options[opt_idx].optname = TCP_USER_TIMEOUT;
            ka_sockopt_params->options[opt_idx].optval  = &tcp_timeout;
            ka_sockopt_params->options[opt_idx].optlen  = sizeof(tcp_timeout);
        }
        ka_sockopt_params->cnt = opt_idx + 1;
    }
    #endif	/* PJ_SOCKET_KEEP_ALIVE_ENABLE */
    //add by james
